...
public sealed class AttributionEngine
{
        ...
        string? url, string? profileLabel, DateTimeOffset now)
    {
        ...
        // — switching from the Client A profile to the plain one kept showing that
        // project for 20 s. Browser windows must also be the same profile,
    ...
    private static string? MatchExplicit(TrackerConfig cfg, string app, string title, string? url, bool isBrowser)
    {
        foreach (var p in cfg.Projects)
        {
            ...
            // Without it a project domain leaked into EVERY app: "mail.exemplu.eu" made any
    ...
    private static string? MatchProfile(TrackerConfig cfg, string app, string title, string aumid, string? label)
    {
        ...
        foreach (var p in cfg.Projects)
        {
            foreach (var frag in p.BrowserProfiles)
            {
                ...
                // Edge puts the profile display name in the window title (confirmed in testing
    ...
    /// name instead: files called "CLIENTC_PREZENTARE" landed on the project whose profile
    /// is "Client C", no matter which profile had them open (2026-08-04). Note Edge writes a
    /// zero-width space inside "Microsoft Edge" — it shows up in practice, hence the strip.
    /// </summary>
