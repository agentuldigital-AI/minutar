...
public sealed class ClassificationRulesTests
{
    private static TrackerConfig BaseConfig()
    {
        ...
        cfg.Classification.Rules.Add(new ClassificationRule { Class = "unproductive", Match = "domain", Value = "memebox.example" });
        return cfg;
    }
    ...
    public void FrontInsertedProductiveTitleRule_BeatsEarlierUnproductiveTitleRule()
    {
        ...
        var title = "facebook — grup fotografie";
    ...
    public void ProductiveDomainRule_ReplacingTheMatchedRule_FlipsTheWholeDomain()
    {
        ...
        Assert.Equal("unproductive", engine.Classify(cfg, "chrome.exe", "MemeBox", "https://memebox.example/hot", null).Class);

        cfg.Classification.Rules.RemoveAll(r => r.Match == "domain" && r.Value == "memebox.example");
        cfg.Classification.Rules.Insert(0, new ClassificationRule { Class = "productive", Match = "domain", Value = "memebox.example" });
        Assert.Equal("productive", engine.Classify(cfg, "chrome.exe", "MemeBox", "https://memebox.example/hot", null).Class);
    }
