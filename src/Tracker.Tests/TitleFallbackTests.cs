...
public sealed class TitleFallbackTests
{
    private static TrackerConfig Config()
    {
        ...
        {
            Name = "Client Mail",
            Domains = new List<string> { "mail.exemplu.eu" },
        });
        cfg.Projects.Add(new ProjectConfig
        {
            Name = "general",
    ...
    [Theory]
    // a subdomain must NOT collapse to its prefix: "mail.exemplu.eu" is not "mail"
    [InlineData("chrome.exe", "Mail - Ana Pop - Outlook")]
    ...
    {
        var project = new AttributionEngine().Attribute(
            Config(), "chrome.exe", "Inbox", aumid: "", url: "https://mail.exemplu.eu/inbox",
            profileLabel: null, DateTimeOffset.UtcNow);

        Assert.Equal("Client Mail", project);
    }
    ...
    [InlineData("CLIENTC_PREZENTARE - Profile 1 - Microsoft Edge")]
    ...
    [InlineData("chrome.exe", "CLIENTC_PREZENTARE - Google Chrome")]
    [InlineData("chrome.exe", "General Motors - stiri")]
    [InlineData("msedge.exe", "Raport generale - Microsoft Edge")]
    ...
    public void ReportAndLiveAgree_WhenTheExtensionLabelIdentifiesTheProfile()
    {
        ...
        var report = AttributionEngine.Resolve(
            cfg, "chrome.exe", "CLIENTC_PREZENTARE", aumid: "", url: null, profileLabel: "Client C");
        var live = new AttributionEngine().Attribute(
            cfg, "chrome.exe", "CLIENTC_PREZENTARE", aumid: "", url: null, profileLabel: "Client C",
    ...
    public void ReportAndLiveAgree_OnAnExplicitKeyword()
    {
        ...
        var report = AttributionEngine.Resolve(
            cfg, "chrome.exe", "CLIENTC_PREZENTARE - Google Chrome", aumid: "", url: null, profileLabel: null);
    ...
    public void Hold_DoesNotCarryTheProjectIntoAnotherBrowserProfile()
    {
        ...
        Assert.Equal("general", att.Attribute(
            cfg, "chrome.exe", "Bookings", "Chrome.UserData.Profile2", null, "General", t0));
    ...
    {
        var project = new AttributionEngine().Attribute(
            Config(), "chrome.exe", "Bookings", aumid: "", url: null, profileLabel: "General",
            DateTimeOffset.UtcNow);
