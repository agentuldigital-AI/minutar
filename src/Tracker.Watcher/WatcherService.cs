...
public sealed class WatcherService : IDisposable
{
    ...
    private async Task WindowLoopAsync(CancellationToken ct)
    {
        ...
        // diagnosticat în testare (goluri de 3-60s în plină activitate — sursă necunoscută):
    ...
    private async Task AfkLoopAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                // PC dormant (sleep / Modern Standby freezes the whole process — a locked
                // span of half an hour left a hole in BOTH stores while this loop was
                // still alive): a clock jump between ticks means nothing could be
